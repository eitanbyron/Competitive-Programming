//
// Created by barbe on 31/05/2023.
//
#include <bits/stdc++.h>
using namespace std;
...
int main() {
    int num_cases;
    cin >> num_cases;
    cin.ignore(); // Ignore the newline character after num_cases

    for (int i = 0; i < num_cases; i++) {
        string str;
        cin >> str;
        SuffixArray sa(str.c_str(), str.size()+1);
        int repeated_substr = 0;
        for (int j = 1; j < sa.LCP.size(); ++j) {
            int cur = max(sa.LCP[j] - sa.LCP[j-1], 0);
            repeated_substr+=cur;
        }

        std::cout << repeated_substr << std::endl;
    }
