...
double minDistPoints(vector<point> v1,vector<point> v2){
    double min= dist(v1[0],v2[0]);
    for(int i=0;i<v1.size()-1;i++){
        for(int j=0;j<v2.size()-1;j++){
            if(dist(v1[i],v2[j])<min) min=dist(v1[i],v2[j]);
        }
    }
    return min;
}
double minDistLines(vector<point> v1,vector<point> v2){
    line2 line;
    line2 perpendicular;
    point intersect;
    double min= dist(v1[0],v2[0]);
    for(auto & i : v1){
        for(int j=0;j<v2.size()-1;j++){
            pointsToLine2(v2[j],v2[j+1],line);
            double a=-1/line.m;
            perpendicular.m=a;
            perpendicular.c=a*i.x-i.y;
            intersect.x=(perpendicular.c-line.c)/(line.m-perpendicular.m);
            intersect.y=intersect.x*line.m+line.c;
            if(dist(intersect,i)<min) min=dist(intersect,i);
        }
    }
    return min;
}
int main() {
    ...
    for(int h=0;h<cases;h++){
        ...
        for(int i=0;i<nIn;i++){
            cin>>x>>y;
            inPol.emplace_back(x,y);
        }
        cin>>nOut;
        for(int i=0;i<nOut;i++){
            cin>>x>>y;
            outPol.emplace_back(x,y);
        }
        inPol.emplace_back(inPol[0].x,inPol[0].y);
        outPol.emplace_back(outPol[0].x,outPol[0].y);
        point c;
        long double distence;
        long double min;
        min= dist(inPol[0],outPol[0]);
        for(int i=0;i<nIn;i++){
            for(int j=0;j<nOut;j++){
                distence=distToLineSegment(inPol[i],outPol[j],outPol[j+1],c);
                if(min>distence) min=distence;
            }
        }
        for(int i=0;i<nOut;i++){
            for(int j=0;j<nIn;j++){
                distence=distToLineSegment(outPol[i],inPol[j],inPol[j+1],c);
                if(min>distence) min=distence;
            }
        }
        min*=0.5;
