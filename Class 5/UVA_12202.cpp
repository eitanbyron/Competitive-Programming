...
bool in_borders(int w,int h,int i,int j){
    if(j<0) return false;
    if(j>=w*h) return false;
    if(i+1==j &&j%w==0) return false;
    if(i-1==j &&i%w==0) return false;
    return true;
}
int main() {
    int W, H;
    ...
    while (!(W == 0 && H == 0)) {
        ...
        for (int i = 0; i < g; i++) {
            cin >> x >> y;
            graph[W * y + x].emplace_back(-1, 0);
        }
        cin >> e;
        for (int i = 0; i < e; i++) {
            cin >> x1 >> y1 >> x2 >> y2 >> t;
            graph[y1 * W + x1].emplace_back(y2 * W + x2, t);
        }
        vector<int> ne(4);
        for (int i = 0; i < W * H-1; i++) {
            if ((!graph[i].empty())/*&& graph[i].back()== make_pair(-1, 0)*/) {
                continue;
            }
            ne[0]=i-1;
            ne[1]=i+1;
            ...
            ne[3]=i+W;
            for (int j: ne) {
                if (in_borders(W,H,i,j) && (graph[j].empty()||graph[j][0] != make_pair(-1, 0))) {
        ...
        bool changed = BellmanFord(graph, 0, d);
        if (changed) cout << "Never" << endl;
        else {
            if (d[W * H - 1] == INF) cout << "Impossible" << endl;
            else cout << d[W * H - 1] << endl;
