...
int main() {
    ...
    int a, b;
    int input_counter;
    int final_a, final_b,final_index;
    while(cin >> m >> n){
        vvi graph (m);
        ...
        for (int i = 0; i < n; ++i) {
            cin >> a >> b;
            graph[a].push_back(b);
            input[make_pair(a,b)]=input_counter;
            input_counter++;
        }
        cout<<"Case "<<index<<": ";
        index++;
        vi compo (m);
        int scc = findSCC(graph, compo);
        if(scc == 1){
            cout << "valid" << endl;
            ...
            for (int i = 0; i < m; ++i) {
                for (auto j = graph[i].begin(); j < graph[i].end(); ++j) {
                    ...
                    graph[i].erase(j);
                    graph[b].push_back(a);
                    scc= findSCC(graph,compo);
                    if(scc==1&&final_index>input[make_pair(a,b)]){
                        final_index=input[make_pair(a,b)];
                        final_a=a;
                        final_b=b;
                        finished= true;
                    }
