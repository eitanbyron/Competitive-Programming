...
int main() {
    ...
    while (n!=0){
        ...
        int maxY=0,minY=0;
        for(int i=0;i<points.size();i++){
            if(points[i].y>points[maxY].y) maxY=i; //max
            if(points[i].y<points[minY].y) minY=i; //min
        }
        double angle=0;
        double minDist=points[maxY].y-points[minY].y;
        for(int i=0;i<points.size();i++){
            double slot=0;
            for(int j=0;j<points.size();j++){
                double dist;
                point p;
                dist= distToLine(points[j],points[i],points[(i+1) %points.size()],p);
                if(dist>slot) slot=dist;
            }
            if(minDist>slot) minDist=slot;
        }
        cout<<"Case "<<testcase<<": ";
        cout<<minDist<<endl;
        testcase++;
